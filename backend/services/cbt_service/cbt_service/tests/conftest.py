...
from cbt_service.main import app
from cbt_service.database.database import database
...
def mock_jwt(monkeypatch):
    ...
    monkeypatch.setattr("cbt_service.dependencies.jwt.decode", fake_decode)
...
def mock_redis(monkeypatch):
    ...
    monkeypatch.setattr("cbt_service.core.redis.redis_client.redis_client", mock)
    monkeypatch.setattr("cbt_service.api.v1.routes.subjects.redis_client", mock)
    yield store
...
def mock_auth_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "cbt_service.clients.auth_client.AuthClient.get_users_bulk",
        fake_get_users_bulk,
    )
