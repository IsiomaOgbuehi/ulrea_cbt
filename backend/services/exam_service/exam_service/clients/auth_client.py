...
from exam_service.schemas.schemas import UserSummary
...
class AuthClient:
    ...
    Uses Redis to cache lookups and avoid hammering auth service.
    """
    ...
    async def get_users_bulk(self, user_ids: list[UUID]) -> dict[str, UserSummary]:
        """Fetch multiple users in one call — avoids N+1."""
        results = {}
        uncached_ids = []
